...
open class SKPaymentQueue: NSObject {
  ...
  fileprivate func succeed() {
    ...
    let product = SKProductsRequest.simulatedProductForIdentifier(ident: self.currentTransaction!.payment.productIdentifier)
    ...
    let plist = [
      ...
      "ProductType": product!.simulatedProductType.rawValue
    ] as [String: Any]
  ...
  open func restoreCompletedTransactions() {
    ...
    let storedTransactions = UserDefaults.standard.array(forKey: "SKTransactions")
